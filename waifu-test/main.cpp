...
int main(int argc, char *argv[]) {
    ...
    Net net = readNetFromDarknet("/home/daniil/CLionProjects/WaifuDetect/yolov3.cfg",
            "/home/daniil/CLionProjects/WaifuDetect/yolov3.weights");
    Mat frame, blob;
    ...
    while (cap.isOpened()) {
        cap >> frame;
        resize(frame, frame, Size(1280, frame.rows * 1280 / frame.cols));

        blobFromImage(frame, blob, 1.0 / 255, Size(yoloWidth, yoloHeight),
...
void removeBox(Mat &frame, const vector<Mat> &out) {
    ...
    for (int idx : indices) {
        Rect box = boxes[idx];
        peopleCount += drawBoxIfPerson(classIds[idx], confidences[idx], box.x, box.y,
                box.x + box.width, box.y + box.height, frame);
    }

    putText(frame, string("people count = ") + to_string(peopleCount), Point(8, 16),
...
bool drawBoxIfPerson(int classId, float conf, int left, int top, int right, int bottom, Mat &frame) {
    ...
    rectangle(frame, Point(left, top - round(1.5 * labelSize.height)),
            Point(left + round(1.5 * labelSize.width), top + baseLine), boxColor, FILLED);
